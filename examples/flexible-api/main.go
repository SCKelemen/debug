...
func (db *DatabaseService) ValidatePassword(userID, password string) bool {
	...
	mc.Error(fmt.Sprintf("Password validation failed for user: %s", userID), debug.WithFlag(SecurityCheck))
...
func main() {
	...
	fmt.Println("4. Clean API: mc.Debug(\"message\") or mc.Debug(\"message\", debug.WithFlag(AdditionalFlag))")
