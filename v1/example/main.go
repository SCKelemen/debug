...
func main() {
	...
	flagDefinitions := []v1.FlagDefinition{
	...
	dm.LogWithSeverity(1<<0, v1.SeverityInfo, "", "HTTP request info")     // Won't log (http.* only shows ERROR)
	dm.LogWithSeverity(1<<0, v1.SeverityError, "", "HTTP request failed")  // Will log
	dm.LogWithSeverity(1<<2, v1.SeverityInfo, "", "DB query info")         // Won't log (db.* shows WARN+)
	dm.LogWithSeverity(1<<2, v1.SeverityWarning, "", "DB query slow")      // Will log
	dm.LogWithSeverity(1<<7, v1.SeverityInfo, "", "Validation passed")     // Will log
	dm.LogWithSeverity(1<<7, v1.SeverityWarning, "", "Validation warning") // Won't log (validation only shows INFO|ERROR)
	fmt.Println()
